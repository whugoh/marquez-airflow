...
class MarquezDag(DAG):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._jobIdMapping = JobIdMapping()
    ...
    def create_dagrun(self, *args, **kwargs):
        ...
        self._jobIdMapping.set(JobIdMapping.make_key(run.dag_id, run.run_id), mqz_job_run_id)
        return run
    ...
    def report_jobrun(self, run_args, execution_date):
        ...
        self.get_mqz_client().set_namespace(self.mqz_namespace)
        self.get_mqz_client().create_job(job_name, self.mqz_location, self.mqz_input_datasets,
                                         self.mqz_output_datasets, self.description)
        mqz_job_run_id = str(self.get_mqz_client().create_job_run(
            job_name, job_run_args=job_run_args, nominal_start_time=start_time, nominal_end_time=end_time).run_id)
        self.get_mqz_client().mark_job_run_running(mqz_job_run_id)
    ...
    def report_jobrun_change(self, dagrun, **kwargs):
        mqz_job_run_id = self._jobIdMapping.pop(JobIdMapping.make_key(dagrun.dag_id, dagrun.run_id))
        if mqz_job_run_id:
